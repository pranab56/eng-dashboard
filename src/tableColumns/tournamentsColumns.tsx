...
export const getTournamentsColumns = (
      ...
      return (
        <div className="flex items-center gap-3 py-1">
          ...
          <div className="flex flex-col max-w-[280px]">
            <span className="font-bold text-gray-900 leading-tight line-clamp-1">
              {tournament.title}
            </span>
      ...
      return (
        <div className="flex items-center gap-1.5 flex-wrap max-w-[260px]">
          {rewards.slice(0, 3).map((r, idx) => (
            ...
            >
              <span className="font-bold text-amber-600">#{r.position}</span>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div
          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border inline-block ${badgeStyle}`}
        >
          {status}
        </div>
      );
