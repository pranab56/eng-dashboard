    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          disabled={isUpdating}
          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1.5 transition-all cursor-pointer disabled:opacity-50 ${badgeStyle}`}
      ...
      <PopoverContent className="w-36 p-1 bg-white border border-gray-200 shadow-xl rounded-xl z-50">
        {[
          { label: "Approved", value: "approved", color: "text-emerald-600 font-bold" },
          { label: "Rejected", value: "rejected", color: "text-red-600 font-bold" },
          { label: "Pending", value: "pending", color: "text-amber-600 font-bold" },
        ].map((opt) => (
          <button
            key={opt.value}
            type="button"
            ...
            className={`w-full px-3 py-2 text-xs rounded-lg flex items-center justify-between transition-colors cursor-pointer text-left ${
              status === opt.value
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2.5 py-1 max-w-[240px]">
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-bold text-gray-900 text-xs leading-snug truncate">
      ...
      return (
        <div className="flex items-center justify-end gap-2 pr-2">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 text-xs font-bold transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            title="Approve Claim"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 text-xs font-bold transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            title="Reject Claim"
          >
